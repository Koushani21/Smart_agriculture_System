...
function displayResult(crop, soil, area, suitabilityScore) {
  ...
  if (sensorValues && barChart && radarChart && doughnutChart) {
    barChart.data.datasets[0].data = [sensorValues.soilMoisture, sensorValues.temperature, sensorValues.humidity, sensorValues.rainfall];
    barChart.update();

    radarChart.data.datasets[1].data = [sensorValues.soilMoisture, sensorValues.temperature, sensorValues.humidity, sensorValues.rainfall];
    radarChart.options.plugins.title.text = `Crop Suitability: ${crop}`;
    radarChart.update();

   
    const otherCrops = Object.keys(cropSensorRanges).filter(c => c !== crop);
    const randomCrops = [];
    while (randomCrops.length < 3 && otherCrops.length > 0) {
      const randomIndex = Math.floor(Math.random() * otherCrops.length);
      randomCrops.push(otherCrops.splice(randomIndex, 1)[0]);
    }
    const allCrops = [crop, ...randomCrops];
    const suitabilityData = allCrops.map(c => c === crop ? suitabilityScore : Math.floor(50 + Math.random() * 30)); // Selected crop gets actual score, others get random 50-80%

    doughnutChart.data.labels = allCrops;
    doughnutChart.data.datasets[0].data = suitabilityData;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const sensorInterval = setInterval(() => {
    ...
    if (sensorValues && barChart && radarChart && currentCrop) {
      barChart.data.datasets[0].data = [sensorValues.soilMoisture, sensorValues.temperature, sensorValues.humidity, sensorValues.rainfall];
      barChart.update();
      radarChart.data.datasets[1].data = [sensorValues.soilMoisture, sensorValues.temperature, sensorValues.humidity, sensorValues.rainfall];
      radarChart.update();
  ...
  resetButton.addEventListener('click', () => {
    ...
    if (sensorValues && barChart && radarChart && doughnutChart) {
      ...
      barChart.update();
      radarChart.data.datasets[1].data = [sensorValues.soilMoisture, sensorValues.temperature, sensorValues.humidity, sensorValues.rainfall];
      radarChart.options.plugins.title.text = 'Crop Suitability: Default';
      radarChart.update();
      doughnutChart.data.labels = ['Tomato', 'Carrot', 'Spinach', 'Rice'];
      doughnutChart.data.datasets[0].data = [90, 75, 60, 40];
      doughnutChart.update();
    }
  ...
  barChart = new Chart(barCtx, {
    type: 'bar',
    data: {
      labels: ['Soil Moisture', 'Temp', 'Humidity', 'Rainfall'],
      datasets: [{
